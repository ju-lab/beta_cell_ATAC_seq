...
min6_d=MIN6_peak_annotated_20180411.bed # differential peak annotated
...
read_tsv("$min6", col_names = F, col_types = "ciicdddciicccciicccciiciiiddi",
		 ...
		 mapped_area_lower = pmax(0, (X24+X25) %/% 2 - 250000), #prevent negative coord
		 mapped_area_upper = (X24+X25) %/% 2 + 250000) %>%
...
sed -i 's/^NA\tNA\tNA/NA\t0\t0/' $min6_h
bedtools intersect -a $min6_h -b $gwas_dm_hg19 -c > $min6_hc
...
colnames(mydata) <- c("mmdifp_chr", "mmdifp_start", "mmdifp_end", "peakid", "fc",
	...
	"phastcons_mm_overlap", "dmloci_count_within_500kb", "difpeak")
mydata %>%
	write_tsv("$min6_d")
EOF
